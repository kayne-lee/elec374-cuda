...
void init_rand_matrix(float* mat, int rows, int cols) {
...
void run_test(int m, int k, int n) {
    ...
    init_rand_matrix(host_a, m, k);
    init_rand_matrix(host_b, k, n);
    ...
    printf("verify: %s\n", verify_result(host_a, host_b, host_res, m, k, n) ? "test PASSED" : "Test FAILED");
